...
FRESULT f_timestamp(const TCHAR* path, u16 f_year, u8 f_month, u8 f_day, u8 f_hour, u8 f_minute, u8 f_second)    /* Pointer to the file name */
{
	FILINFO fno;
...
FRESULT f_record_init(TCHAR* path)
{
	...
	if (f_res == FR_OK)
	{
		f_res = f_timestamp(path,2019,2,11,11,58,0);
...
FRESULT f_record(const TCHAR* path, FIL* fp, const TCHAR* fmt,	...)
{
	...
	if (f_res == FR_OK)
	{
		...
		f_res = f_timestamp(path,2019,3,14,22,13,0);
	}
